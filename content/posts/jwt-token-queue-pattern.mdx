...
이 글은 네 가지 문제를 PaintLater가 실제 코드에서 어떻게 풀었는지 하나씩 살펴보는 기록입니다. 결론부터 말하면 `BroadcastChannel`이나 Web Locks API 같은 크로스 탭 통신 API 없이도 멀티탭 문제를 실용적으로 해결할 수 있습니다. 비결은 **서버와 프론트가 역할을 잘 나누는 것**, 단 한 줄입니다.
...
코드에 주석은 없습니다. 경험적으로 결정된 숫자입니다. 일반적인 브라우저의 쿠키 업데이트는 수 밀리초 안에 끝나지만, 다음을 감안한 여유값이 포함되어 있습니다.
...
각각이 커버하는 레이스가 다릅니다. "플래그 하나로 통합하면 깔끔하지 않을까" 하고 손대는 순간 **거의 확실히 미묘한 버그가 돌아옵니다**. 경험에서 나온 경고입니다.
...
이 그림이 `client.ts:266-368`의 본문과 1:1로 대응합니다. 읽기가 복잡한 건 **코드가 잘못 짜여서가 아니라 문제 자체가 복잡해서**입니다. 각 분기가 실제 경합 시나리오 하나씩을 막고 있습니다.
...
이 구현을 뜯어보면서 가장 인상 깊었던 건 **"쓰지 않기로 한 것"** 이 이렇게 많다는 점이었습니다.
...
각 요소는 지극히 평범합니다. 조합의 순서와 역할 분담이 전부입니다. 복잡한 문제에 대한 단순한 해법은 **도구를 많이 쓰는 것이 아니라 책임을 잘 나누는 것**이라는 걸 다시 한 번 확인한 시간이었습니다.
