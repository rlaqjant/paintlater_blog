...
흐름을 한 줄로 요약하면 — **LLM은 "색 방향"을 제안하기만 하고, "실제 페인트"는 결정론적 Java 엔진이 고릅니다.** 이 역할 분리가 이 글의 핵심 서사입니다.
...
AI와는 별개로, 이런 결정론적 수치 기반 레이어는 **백엔드에 단 한 번만 존재합니다**. 프론트엔드에는 CIEDE2000 구현이 없고, 서버가 계산해 준 `deltaE` 값을 표시만 합니다. 덕분에 카탈로그가 바뀌어도 클라이언트를 건드릴 필요가 없습니다 — 진리원이 한 군데라는 건 이런 상황에서 항상 이깁니다.
...
이게 이 전체 파이프라인의 가장 중요한 한 줄입니다.

LLM에게 "이 미니어처에 어울리는 시타델 페인트를 추천해줘"라고 물어보는 건 **매력적인 실패 경로**입니다. 왜냐하면:
...
이 분리의 결과, **LLM이 환각으로 만든 페인트 이름은 사용자 화면까지 도달할 경로가 존재하지 않습니다**. "가장 가까운 카탈로그 페인트" 판정은 언제나 `paintConversionService.matchByHex(...)`가 내리고, 이 함수는 실제로 존재하는 페인트 엔트리만 반환합니다.
...
이 lenient 정책의 장점은 명백합니다. **사용자는 항상 뭔가 결과를 받습니다.** LLM이 형편없는 응답을 했다고 해서 잡이 FAILED가 되지 않고, 폴백 팔레트로 파이프라인이 계속 흐릅니다.

단점도 명백합니다. **사일런트 품질 저하.** 잡은 DONE으로 끝나지만 사용자가 받은 추천이 회색 3색일 수 있습니다. 다만 응답에는 `confidence` 필드와 `warnings` 배열이 같이 담겨 있어서, UI가 이 값으로 "AI가 확신을 못 한 결과" 신호를 사용자에게 전달할 수 있는 여지는 열려 있습니다.
...
왜 이중 방어인가? 프롬프트는 통계적이고 매처는 결정론적이기 때문입니다. 프롬프트로 "varnish를 언급하지 마라"라고 해도 모델이 한 번씩 반항할 수 있고, 설령 LLM이 hex만 뱉어도 그 hex가 우연히 어떤 varnish 제품의 색과 일치할 수 있습니다. Java 매처가 카탈로그 수준에서 `type != "standard"`를 거르면, **LLM이 뭘 하든 사용자에게 varnish가 추천되는 경로가 없습니다**.
...
이 한 함수에 철학이 압축돼 있습니다. **"수학적으로 가장 가까운 페인트"와 "사용자가 이미 들고 있는 페인트"가 다를 때, ΔE 5 안에서는 보유 페인트가 이깁니다.** 5를 넘어서면 정확도가 이기고.

그리고 매처의 출력이 `buildCandidateNode`(`:232-246`)를 거치는 순간, 사용자 응답에 들어가는 hex/brand/name은 **전부 카탈로그에 실재하는 페인트의 값**입니다. LLM이 무슨 hex를 뱉었든, 그건 이 단계에서 **"검색 키"로 소비**되고 버려집니다. 환각이 사용자에게 도달할 경로가 없습니다.
...
이렇게 하면 **DB에 확실히 커밋된 잡만 워커가 본다**는 불변식을 만들 수 있습니다. 부모가 롤백되면 `afterCommit`은 호출조차 되지 않고, 비싼 OpenAI 호출은 시작도 안 됩니다.
...
조건부 UPDATE입니다. 반환값이 0이면 이미 다른 워커가 처리 중이거나 완료된 상태이므로 **즉시 리턴**합니다. 이걸로 워커 수준의 멱등성을 확보합니다 — timeout scheduler가 hang된 잡을 재처리하려 할 때 같은 잡이 두 번 실행되는 걸 막는 마지막 장벽입니다.
...
이 파이프라인을 만들면서 제일 오래 고민한 건 "LLM에게 어디까지 맡길 것인가"였습니다. 그리고 지금 돌아보면, 실제로 깎아낸 답은 의외로 단순합니다. **LLM의 출력 도메인을 최대한 좁히고, 나머지는 결정론적 코드가 맡는다.**
...
이 다섯 가지는 서로 다른 층위에 있습니다. 프롬프트 수준의 방어, 파싱/검증 수준의 방어, 도메인 로직 수준의 방어, 트랜잭션/동시성 수준의 방어, 스케줄러 수준의 방어. 어느 한 층이 뚫려도 다음 층이 있습니다. 그리고 이 중 어느 것도 LLM **내부**를 더 잘 이해하는 문제가 아닙니다. **전부 모델 바깥의 평범한 소프트웨어 엔지니어링 문제**입니다. LLM은 새로운 실패 유형을 몇 개 추가했을 뿐이고, 그걸 다루는 도구는 이미 우리 손에 있었습니다 — 상태 머신, 트랜잭션 경계, 결정론적 검증, 타임아웃, 스케줄러. 전부 익숙한 것들입니다.

그래서 이 글의 결론은 한 문장입니다. **AI를 모듈로 쓸 때 가장 중요한 설계 결정은 "AI에게 무엇을 시킬 것인가"가 아니라 "AI에게 무엇을 시키지 않을 것인가"다.** 도메인을 좁히는 순간, 나머지는 익숙한 엔지니어링이 됩니다.
